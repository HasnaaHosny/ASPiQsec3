...
class _ReportDetailScreenState extends State<ReportDetailScreen> {
  ...
  String _formatReportTitle(String groupName) {
    switch (groupName) {
      case "فرحان":
        return "تقرير الفرح";
      case "حزين":
      ...
      case "مندهش":
        return "تقرير الدهشة";
      case "يفكر":
        return "تقرير التفكير";
      case "التواصل اللفظي":
        return "تقرير التواصل اللفظي";
      case "التواصل غير اللفظي":
        return "تقرير التواصل غير اللفظي";
      default:
  ...
  Future<void> _generateAndSharePdf() async {
    ...
    try {
      ...
      final String dynamicReportTitle =
          _formatReportTitle(widget.reportGroup.groupName);

      final File pdfFile = await _pdfGeneratorService.generateReportPdf(
        widget.reportGroup,
        _fetchedProgressData,
      );
      await _pdfShareService.sharePdf(pdfFile, subject: dynamicReportTitle);
    } catch (e) {
        ...
        appBar: PreferredSize(
          ...
          child: Padding(
            ...
            child: AppBar(
              ...
              iconTheme: IconThemeData(color: _unifiedColor),
              title: Column(
                children: [
                  Text(
                    dynamicReportTitle,
                  ...
                  Text(
                    'الطفل: ${widget.reportGroup.childName}',
                    style: TextStyle(
                      color: _unifiedColor,
                      fontSize: 14,
                    ),
                  ...
                  Text(
                    'الأخصائي: ${widget.reportGroup.doctorName}',
                    style: TextStyle(
                      color: _unifiedColor,
                      fontSize: 14,
                    ),
                ...
                if (_isProcessingPdf)
                  Padding(
                    padding: const EdgeInsets.symmetric(
                        horizontal: 16.0), // تعديل الـ padding
                    child: SizedBox(
                        width: 24, // تعديل الحجم ليتناسب
                        height: 24,
                        child: CircularProgressIndicator(
                            strokeWidth: 3, color: _unifiedColor)),
                  )
                else
                  IconButton(
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  _buildTrainedEmotionsSection(
                      context,
                      widget.reportGroup.good.items,
                      widget.reportGroup.notGood.items),
                  const SizedBox(height: 24),
                  _buildSectionTitle(context, 'ملخص أداء الطفل في الاختبار'),
                  _buildPerformanceSummarySection(
                      context,
                      widget.reportGroup.good.items,
                      widget.reportGroup.notGood.items),
                  const SizedBox(height: 24),
  ...
  Widget _buildInfoCard(BuildContext context, String titleText,
      String valueText, IconData? iconData) {
  ...
  Widget _buildTrainedEmotionsSection(BuildContext context,
    ...
    for (var item in goodItems) {
      emotionWidgets.add(
          _buildEmotionItem(context, '${item.title}: ${item.message}', true));
    }
    for (var item in notGoodItems) {
      emotionWidgets.add(
          _buildEmotionItem(context, '${item.title}: ${item.message}', false));
    }
  ...
  Widget _buildPerformanceSummarySection(BuildContext context,
    ...
    for (var item in goodItems) {
      summaryWidgets.add(
          _buildPerformanceItem(context, '${item.title}: ${item.message}'));
    }
    for (var item in notGoodItems) {
      summaryWidgets.add(
          _buildPerformanceItem(context, '${item.title}: ${item.message}'));
  ...
  Widget _buildRecommendationsSection(
    ...
    if (recommendationItems.isEmpty) {
      return Padding(
          padding: const EdgeInsets.symmetric(vertical: 8.0),
          child: Text('لا توجد توصيات لعرضها.',
              style: TextStyle(color: _unifiedColor),
              textDirection: TextDirection.rtl));
    }
