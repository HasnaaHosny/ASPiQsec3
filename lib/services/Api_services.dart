...
class ApiService {
  ...
  static Future<Session?> getNextPendingSession(String token) async {
    ...
    try {
      ...
      print(
          '[ApiService.getNextPendingSession] Response Body: ${response.body}'); // مهم جداً
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (data != null && data is Map<String, dynamic>) {
            ...
            if (data.containsKey('message') &&
                !data.containsKey(
                    'details') && // جلسة كاملة عادة ما تحتوي 'details'
                !data.containsKey('newDetail')) {
              // جلسة كاملة عادة ما تحتوي 'newDetail'

              String message = data['message'] as String? ?? '';
              print(
                  '[ApiService.getNextPendingSession] Detected a special message: $message');
              ...
              Map<String, dynamic> sessionLikeMessage = {
                "id": data['sessionId'] as int? ??
                    0, // أو قيمة مناسبة إذا لم يكن sessionId موجودًا
                ...
                "detailsCount": 0,
                "attributes": {
                  // هنا نضع الرسائل المستخرجة
                  if (message.toLowerCase().contains("اختبار الشهر"))
                    "monthly_test_message": message,
                  if (message.toLowerCase().contains("اختبار ال 3 شهور"))
                    "three_month_test_message": message,
                },
            ...
            // إذا لم تكن رسالة خاصة، افترض أنها بيانات جلسة كاملة
              ...
              // تحقق من وجود حقل أساسي للجلسة
              print(
                  '[ApiService.getNextPendingSession] Successfully fetched full session data.');
              return Session.fromJson(data);
            }
            // إذا لم تكن أيًا مما سبق، فهي بنية غير متوقعة
            else {
              print(
                  '[ApiService.getNextPendingSession] Unknown data structure received, but it is a Map.');
              return null;
            }
          } else if (data == null || (data is List && data.isEmpty)) {
            print(
          ...
          print(
              '[ApiService.getNextPendingSession] JSON Parsing Error: $e. Body was: ${response.body}');
          return null;
        }
      } else if (response.statusCode == 404) {
        print(
        ...
        throw Exception(
            'Failed to load next session (Status: ${response.statusCode})'); // سيتم التقاطها في HomeScreen
      }
    } on TimeoutException catch (e) {
  ...
  static Future<bool> submitAllAssessmentAnswers(
    ...
    const String bulkSubmitApiUrl =
        'http://aspiq.runasp.net/api/sessioncontroller/submit-answers'; // <<--- غيّر هذا الرابط ليتناسب مع الـ endpoint الجديد
    // أو إذا كان نفس الرابط: 'http://aspiq.runasp.net/api/sessioncontroller/submit-answers'

    print(
        "[ApiService.submitAllAssessmentAnswers] Attempting to submit ${answersToSubmit.length} answers to URL: $bulkSubmitApiUrl");
    ...
    try {
      ...
      print(
          "[ApiService.submitAllAssessmentAnswers] Submitting JSON: $jsonPayload");
      ...
      final response = await http
          .post(
            ...
            headers: {
              ...
              'Authorization':
                  'Bearer $jwtToken', // عدّل هذا إذا كانت طريقة المصادقة مختلفة
            },
            body: jsonPayload,
          )
          .timeout(const Duration(
              seconds: 30)); // إضافة مهلة 30 ثانية لعملية الإرسال المجمع

      if (response.statusCode == 200 || response.statusCode == 201) {
        // 200 OK أو 201 Created عادة ما يشيران إلى النجاح
        print(
            '[ApiService.submitAllAssessmentAnswers] Bulk answers submitted successfully. Status: ${response.statusCode}');
        return true;
      } else {
        print(
            '[ApiService.submitAllAssessmentAnswers] Failed to submit bulk answers. Status: ${response.statusCode}');
        print(
            '[ApiService.submitAllAssessmentAnswers] Response body: ${response.body}');
        return false;
      }
    } on TimeoutException catch (e) {
      print(
          "[ApiService.submitAllAssessmentAnswers] Error: Bulk submission API request timed out - $e");
      return false;
    } catch (e) {
      print(
          '[ApiService.submitAllAssessmentAnswers] Error submitting bulk answers: $e');
      return false;
    }
  }
  ...
  */
  ...
  static Future<Map<String, dynamic>> verifyOtp(String identifier, String code,
    ...
    try {
      final payload = {'token': identifier, 'otpCode': code};
...
// ---!!! دالة جديدة لتعليم تفصيل الاختبار كـ "غير مكتمل" !!!---
  ...
  static Future<bool> addIncorrectAnswerComment(
      String token, int sessionId) async {
    // تأكدي من أن هذا هو الـ Endpoint والـ baseUrl الصحيحين
    final String endpointUrl =
        '$baseUrl/Reports/add-comment'; // بناءً على الصورة، المسار هو /api/Reports/add-comment
    // إذا كان baseUrl هو http://aspiq.runasp.net/api
    // فالرابط سيكون صحيحًا
    debugPrint(
        "[ApiService.addIncorrectAnswerComment] Calling: POST $endpointUrl for Detail ID: $sessionId");
    ...
    try {
      ...
      debugPrint(
          "[ApiService.addIncorrectAnswerComment] Status: ${response.statusCode}, Body: ${response.body}");

      // عادةً ما يكون 200 OK أو 201 Created أو 204 No Content علامات نجاح
      if (response.statusCode >= 200 && response.statusCode < 300) {
        debugPrint(
            "[ApiService.addIncorrectAnswerComment] Comment added successfully for Detail ID: $sessionId");
        return true;
      } else {
        debugPrint(
            "[ApiService.addIncorrectAnswerComment] Failed to add comment for Detail ID: $sessionId. Status: ${response.statusCode}");
        return false;
      }
    } on TimeoutException catch (e) {
      debugPrint(
          "[ApiService.addIncorrectAnswerComment] Timeout error for Detail ID $sessionId: $e");
      return false;
    } catch (e) {
      debugPrint(
          "[ApiService.addIncorrectAnswerComment] General error for Detail ID $sessionId: $e");
      return false;
    }
  }
