                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    ProgressBarCard(
                      ...
                      description:
                          'تمارين لتطوير مهارات التواصل الفعال مع الآخرين.',
                      endpoint:
                          'http://aspiq.runasp.net/api/Reports/progress3',
                      onTap: () => Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) =>
                                  const SocialSkillsScreen())),
                    ),
                    // ========================================================
                    // <<< --- نهاية الكود المضاف --- >>>
                    // ========================================================
...
import 'package:myfinalpro/widgets/Notifictionicon.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'start_test_screen.dart' as monthly_test_intro_screen;
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadTokenAndInitialData() async {
    ...
    try {
      ...
      if (mounted && !_isFetchingTestGroup && !_isFetchingMonthlyTest && (cooldownTimer == null || !cooldownTimer!.isActive)) {
        setStateIfMounted(() => _isLoading = false);
      }
      await _updateSessionNotifications();
    }
  ...
  Future<void> _fetchNextSessionDataFromApi() async {
    ...
    try {
      ...
      if (_nextSessionData != null) {
        debugPrint("HomeScreen: Fetched _nextSessionData: ID=${_nextSessionData!.sessionId}, Title='${_nextSessionData!.title}', TypeID=${_nextSessionData!.typeId}");
        
        await _checkAndGenerateTestNotificationsBasedOnSessionObject(_nextSessionData!);

        if (_nextSessionData!.typeId != 99 && _nextSessionData!.title != null && _nextSessionData!.title != "رسالة نظام") {
          setStateIfMounted(() {
            isSessionAvailable = true;
            _errorMessage = null;
          });
          ...
          if (_nextSessionData!.attributes == null || (_nextSessionData!.attributes!['monthly_test_message'] == null && _nextSessionData!.attributes!['three_month_test_message'] == null)) {
            String messageToSend = _nextSessionData!.description ?? _nextSessionData!.title ?? "رسالة من النظام.";
            
            final newNotification = notif_model.NotificationItem(
                id: 'system_message_homescreen_${DateTime.now().millisecondsSinceEpoch}',
                title: messageToSend,
                createdAt: DateTime.now(),
                type: notif_model.NotificationType.systemMessage,
            );
      ...
      if (!mounted) return;
      String errorMsg = "خطأ في جلب الجلسة.";
      if (e.toString().contains('Unauthorized')) {
        errorMsg = "انتهت صلاحية الدخول.";
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
  ...
  void _startUiUpdateTimer(int sessionStartTimeReferenceMillis) {
    ...
    updateRemainingTimeCallback() async {
      ...
      if (elapsedMillis >= sessionCooldownDuration.inMilliseconds) {
        cooldownTimer?.cancel();
        if (isSessionInProgressOrCoolingDown) {
           final readyNotification = notif_model.NotificationItem(
  ...
  Future<void> _startSession() async {
    if (!isSessionAvailable || _nextSessionData == null || _jwtToken == null || _nextSessionData!.typeId == 99) {
      if (mounted && !isSessionAvailable && _errorMessage == null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("لا توجد جلسة متاحة.", textDirection: TextDirection.rtl)));
      }
      return;
    }
    try {
  ...
  Future<void> _showInfoDialog(String title, String message) async {
    if (!mounted) return;
    return showDialog<void>(context: context, builder: (BuildContext dialogContext) => AlertDialog(
  ...
  Future<void> _startThreeMonthTest() async {
    ...
    try {
      ...
      if (testGroupData != null && testGroupData.sessions.isNotEmpty) {
        ...
        _showInfoDialog("تنبيه", "لا يوجد اختبار 3 شهور متاح حاليًا.");
      }
    } catch (e) {
      debugPrint("HomeScreen: Error starting 3-month test: $e");
      if (mounted) {
        _showInfoDialog("خطأ", "خطأ تحميل اختبار الـ 3 شهور.");
      }
    } finally {
      if (mounted) setStateIfMounted(() => _isFetchingTestGroup = false);
    }
    ...
    try {
      ...
      result.fold(
        (failure) {
          ...
          _showInfoDialog("تنبيه", failure.message);
        },
        (monthlyTestResponse) {
          if (monthlyTestResponse.details.isNotEmpty || monthlyTestResponse.newDetails.isNotEmpty) {
            Navigator.push(context, MaterialPageRoute(builder: (context) => const monthly_test_intro_screen.Timetest()))
                .then((_) => _loadTokenAndInitialData());
          } else {
            _showInfoDialog("تنبيه", monthlyTestResponse.messageFromApi ?? "لا يوجد تقييم شهري متاح حاليًا.");
          }
        },
      );
    } catch (e) {
      debugPrint("HomeScreen: Unexpected error in _startMonthlyTest: $e");
      if (mounted) {
        _showInfoDialog("خطأ", "حدث خطأ غير متوقع أثناء محاولة بدء التقييم الشهري.");
      }
    } finally {
      if (mounted) setStateIfMounted(() => _isFetchingMonthlyTest = false);
    }
  ...
  void _showLoginRedirectSnackbar() {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("يرجى تسجيل الدخول أولاً.", textDirection: TextDirection.rtl)));
    }
  }

  @override
  Widget build(BuildContext context) {
    bool showAppLoadingIndicator = _isLoading && !isSessionInProgressOrCoolingDown;
        ...
        appBar: AppBar(
          ...
          actions: [
            Padding(
                ...
                child: Builder(
                    builder: (context) => IconButton(
                        icon: const Icon(Icons.menu, color: primaryBlue, size: 32.0),
                        onPressed: () => Scaffold.of(context).openEndDrawer(),
            ...
            : RefreshIndicator(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (_errorMessage != null)
                      Padding(
                          ...
                          child: Center(
                              child: Text(_errorMessage!,
                                  textAlign: TextAlign.center,
                                  style: const TextStyle(color: Colors.redAccent, fontFamily: 'Cairo', fontSize: 16.0, fontWeight: FontWeight.w500)))),
                    Container(
                      width: double.infinity,
                      constraints: const BoxConstraints(minHeight: 112.0),
                      ...
                      decoration: BoxDecoration(
                          color: primaryBlue,
                          borderRadius: BorderRadius.circular(16.0),
                          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 8.0, offset: const Offset(0, 4.0))]),
                      child: AnimatedSwitcher(
                          duration: const Duration(milliseconds: 350),
                          transitionBuilder: (Widget child, Animation<double> animation) => FadeTransition(opacity: animation, child: child),
                          child: isSessionInProgressOrCoolingDown ? _buildCooldownView() : _buildStartSessionView(),
                      ),
                    ),
                    const SizedBox(height: 8.0),
                    Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Expanded(child: _buildAssessmentCard(
                            title: 'التقييم الشهري',
                            subtitle: 'تقييم شهري لمتابعة التطورات خطوة بخطوة.',
                            buttonText: 'ابدأ الاختبار',
                            onPressed: _isFetchingMonthlyTest || _isFetchingTestGroup ? null : _startMonthlyTest,
                            isLoading: _isFetchingMonthlyTest,
                          )),
                          const SizedBox(width: 8.0),
                          Expanded(child: _buildAssessmentCard(
                            title: 'اختبار الـ 3 شهور',
                            subtitle: 'تقييم شامل كل ثلاثة أشهر لقياس التقدم العام.',
                            buttonText: 'ابدأ الاختبار',
                            onPressed: _isFetchingMonthlyTest || _isFetchingTestGroup ? null : _startThreeMonthTest,
                            isLoading: _isFetchingTestGroup,
                          )),
                        ]),
                    const SizedBox(height: 16.0),
                    const Padding(
                        padding: EdgeInsets.only(right: 8.0, bottom: 8.0),
                        child: Text('تطوير المهارات وإدارة الانفعالات', style: TextStyle(fontSize: 20.0, fontWeight: FontWeight.bold, fontFamily: 'Cairo', color: primaryBlue))),
                    ProgressBarCard(
                      title: 'الانفعالات',
                      description: 'تمارين للتعرف على المشاعر وإدارتها بفعالية.',
                      endpoint: 'http://aspiq.runasp.net/api/Reports/progress1',
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const TrainingSessionsScreen())),
                    ),
                    const SizedBox(height: 8.0),
                    ProgressBarCard(
                      title: 'تنمية المهارات',
                      description: 'استكشف تمارين قيمة لتطوير مهاراتك الشخصية والاجتماعية!',
                      endpoint: 'http://aspiq.runasp.net/api/Reports/progress2',
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const TrainingSessionsScreenV2())),
                    ),
                    const SizedBox(height: 8.0),
                    ProgressBarCard(
                      title: 'التواصل الاجتماعي',
                      description: 'تمارين لتطوير مهارات التواصل الفعال مع الآخرين.',
                      endpoint: 'http://aspiq.runasp.net/api/Reports/progress3',
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const SocialSkillsScreen())),
                    ),
                    const SizedBox(height: 40.0),
                  ],
                ),
          ...
          children: [
            Expanded(
                ...
                child: Column(
                    ...
                    children: [
                      Text(
                          (isSessionAvailable && _nextSessionData != null && _nextSessionData!.typeId != 99)
                              ? 'حان وقت جلسة${_nextSessionData!.title != null && _nextSessionData!.title!.isNotEmpty ? " ${_nextSessionData!.title}" : "!"}'
                              : "لا توجد جلسات تدريبية متاحة حاليًا",
                          style: const TextStyle(color: Colors.white, fontSize: 18.0, fontFamily: 'Cairo', fontWeight: FontWeight.bold),
                          textAlign: TextAlign.center),
                      const SizedBox(height: 16.0),
                      ElevatedButton(
                          onPressed: (isSessionAvailable && _nextSessionData != null && _nextSessionData!.typeId != 99 && !_isFetchingTestGroup && !_isFetchingMonthlyTest) ? _startSession : null,
                          style: ElevatedButton.styleFrom(
                              backgroundColor: Colors.white,
                              padding: const EdgeInsets.symmetric(horizontal: 40.0, vertical: 16.0),
                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24.0)),
                              disabledBackgroundColor: Colors.white.withAlpha(178),
                              disabledForegroundColor: primaryBlue.withAlpha(128),
                              elevation: 4.0),
                          child: Text('لنبدأ', style: TextStyle(color: (isSessionAvailable && _nextSessionData != null && _nextSessionData!.typeId != 99 && !_isFetchingTestGroup && !_isFetchingMonthlyTest) ? primaryBlue : Colors.grey.shade400, fontSize: 16.0, fontFamily: 'Cairo', fontWeight: FontWeight.bold)))
                    ])),
            Expanded(
                flex: 4,
                child: Image.asset("assets/images/session.png", height: 96.0, fit: BoxFit.contain, errorBuilder: (ctx, err, st) =>
                    const SizedBox(height: 96.0, child: Center(child: Icon(Icons.image_not_supported, color: Colors.white54, size: 48.0)))))
          ]));
  }
  ...
  Widget _buildAssessmentCard({
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            color: cardBgColor,
            borderRadius: BorderRadius.circular(16.0),
            boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 8.0, offset: const Offset(0, 4.0))]),
        child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(title, style: TextStyle(fontSize: 16.0, fontWeight: FontWeight.bold, fontFamily: 'Cairo', color: textColor)),
                const SizedBox(height: 8.0),
                Text(subtitle, style: TextStyle(color: textColor.withAlpha(191), fontSize: 12.0, fontFamily: 'Cairo', height: 1.3))
              ]),
              const SizedBox(height: 16.0),
              Align(
                  alignment: Alignment.centerLeft,
                  child: ElevatedButton(
                      onPressed: onPressed,
                      child: isLoading
                          ? const SizedBox(width: 16.0, height: 16.0, child: CircularProgressIndicator(strokeWidth: 2.0, color: Colors.white))
                          : Text(buttonText, style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w600, fontSize: 14.0)),
                      style: ElevatedButton.styleFrom(
                          backgroundColor: buttonBgColor,
                          foregroundColor: buttonFgColor,
